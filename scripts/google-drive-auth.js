...
//
// Kjør lokalt:  node scripts/google-drive-auth.js
// (skriptet spør om Client ID og secret — de kan også gis som argumenter)
//
...
async function main() {
  ...
  if (!clientId)     clientId     = await prompt('Lim inn Client ID (slutter på .apps.googleusercontent.com): ');
  if (!clientSecret) clientSecret = await prompt('Lim inn Client secret (starter med GOCSPX-): ');
  if (!clientId || !clientSecret) {
    console.error('Både Client ID og Client secret er påkrevd.');
    process.exit(1);
  }
  if (!clientId.endsWith('.apps.googleusercontent.com'))
    console.warn('\n⚠ Client ID-en slutter ikke på .apps.googleusercontent.com — dobbeltsjekk at du kopierte «Client ID»-feltet fra en OAuth-klient (type Desktop), ikke service accounten.\n');
  ...
  server.listen(0, '127.0.0.1', () => {
    ...
    server.on('request', async (req, res) => {
      ...
      try {
        ...
        console.log('\n✓ Godkjent, og mappen «ORO CRM Backups» er opprettet i din Disk.\n');
        console.log('Kjør denne kommandoen (én linje) for å aktivere opplastingen i Railway:\n');
        console.log(`railway variables --service oro-crm --set "GOOGLE_CLIENT_ID=${clientId}" --set "GOOGLE_CLIENT_SECRET=${clientSecret}" --set "GOOGLE_REFRESH_TOKEN=${tokens.refresh_token}" --set "GOOGLE_DRIVE_FOLDER_ID=${folder.id}"\n`);
      } catch (e) {
        console.error('\nFeilet:', e.message);
        process.exit(1);
      }
